...
//Get Variables:
$data = $_POST;
if (!isset($data["cipherKey"]) || empty($data["cipherKey"]) || $data["cipherKey"] === "") {
    echo json_encode(array("success"=>false, "message"=>"CipherKey was not set!"));
    exit;
}
$key = strtolower($_POST["cipherKey"]);

if (!isset($data["cipherContent"]) || empty($data["cipherContent"]) || $data["cipherContent"] === "") {
    echo json_encode(array("success"=>false, "message"=>"CipherContent was not set!"));
    exit;
}
$content = strtolower($_POST["cipherContent"]);

if (!isset($data["cipherAlphabet"]) || empty($data["cipherAlphabet"]) || $data["cipherAlphabet"] === "") {
    echo json_encode(array("success"=>false, "message"=>"CipherAlphabet was not set!"));
    exit;
}
$alphabetString = $_POST["cipherAlphabet"];

if (!isset($data["ende"]) || empty($data["ende"]) || $data["ende"] === "") {
    echo json_encode(array("success"=>false, "message"=>"Encrypt/Decrypt was not set!"));
    exit;
}
$encDec = $_POST["ende"];
...
if ($encDec === "en") {
    ...
    echo json_encode(array("success"=>true, "message"=>"Your message has been encoded", "data"=>$encrypted));
} elseif ($encDec === "de") {
    $decrypted = decrypt($content, $key, $alphabetTable, $alphabetString);
    echo json_encode(array("success"=>true, "message"=>"Your message has been decoded", "data"=>$decrypted));
} else {
